...
async def take_vote(ctx, question:str, vote_time, min_voters):
    ...
    embed_title="MUTE VOTE"
    votey_message = await ctx.send(
        embed=discord.Embed(
            type="rich",
            title=embed_title,
            description="{0}\n\n✅ - Yes\n❌ - No\nMinimum {1} votes required.Vote time is {2} seconds.".format(question, min_voters,vote_time)
        )
    )
    ...
    while curr_time < vote_time:
        ...
        if all_in_favor > not_in_favor and not_in_favor > 0 and all_in_favor >= min_voters:
            passed = True
            break
